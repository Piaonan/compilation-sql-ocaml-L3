...
*)
  ...
  let daoko t1 t2 = match (t1:tuple), (t2:tuple) with
	| a1, a2 -> ((Array.append a1 a2):tuple)
  ...
  let rightouterjoin pred r1 r2 =
    let card = ref 0 in
    let sch = Array.append r1.sch r2.sch in
    let dummyt2 = Array.make (width r1) None in
    let ext = TSet.fold (fun t2 n2 acc ->
      let somet2 = ref false in
      let acc = TSet.fold (fun t1 n1 acc ->
        if pred t1 t2
        then (
          card := !card + n1 * n2;
          somet2 := true;
      ...
      if !somet2 then acc else (
        card := !card + n2;
        TSet.add (Array.append dummyt2 t2) n2 acc
      )
    ) r2.ext TSet.empty in
    { card = !card; ext = ext; sch = sch } 
